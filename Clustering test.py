...
def LevenshteinDistanceWords(s,t):
    s_array = s.split()
    t_array = t.split()
    return LevenshteinDistance(s_array,t_array)

if LevenshteinDistanceWords("hello i am fin","goodbye i am george") != 2:
    raise Exception("incorrect distance")

if LevenshteinDistance("aa", "ab") != 1:
    raise Exception("incorrect distance")
if LevenshteinDistance("aaaa", "abcd")!= 3:
    raise Exception("incorrect distance")

if LevenshteinDistance(["aaaa","bbbb"],["cccc","dcba"]) !=2:
    raise Exception("incorrect distance")
...
cl = HierarchicalClustering([
    ...
    "Pandas are similar to koalas because they are both specialists. They are different from snakes because snakes are generalist species. Snakes are generalists because they can adapt to many different locations where as pandas are stuck on China and koalas are in Australia.",
    ...
    "Panda from china and koalas from Australia are both specialists.They live eating special plants like bamboo and eucalyptus leaves while pythons are very different.They can eat almost anything and kill only animals as a source of food.",
...
clusters = cl.getlevel(45)

for j in range(len(clusters)):
    print(len(clusters[j]), clusters[j])
